...
const ServiceDetailsPage = () => {
  ...
  const handleSave = async () => {
    await dispatch(
      updateService({ id: editService._id, service: editService })
    );
  ...
  return (
    <div className="md:container md:mx-auto md:max-w-4xl">
      <div className="flex justify-between shadow-lg bg-white text-white p-2 sticky top-0 z-50">
        <span
          onClick={() => {
            router.back();
          }}
        >
          <BiChevronLeft className="inline-flex w-8 h-8 text-black cursor-pointer" />
        </span>
        <div className="flex items-center space-x-8">
          <img src="/images/4.png" alt="log" className="w-20 h-auto" />
        </div>
      </div>
